...
def find_swing_points_v2(df, window=2):
...
def get_recent_swing_low_v2(df, current_idx, lookback=10):
    start_idx = max(0, current_idx - lookback)
    recent_data = df.iloc[start_idx:current_idx+1]
    return recent_data['Low'].min()

def get_recent_swing_high_v2(df, current_idx, lookback=10):
    start_idx = max(0, current_idx - lookback)
    recent_data = df.iloc[start_idx:current_idx+1]
    return recent_data['High'].max()

def backtest_strategy_v2(df):
    trades = []
    position = None
    ...
    for i in range(max(ATR_PERIOD, EMA_LONG) + 5, len(df)):
        ...
        if position is None:
            if (df['EMA9'].iloc[i] > df['EMA20'].iloc[i] and df['Close'].iloc[i] > df['EMA9'].iloc[i] and not pd.isna(df['ATR'].iloc[i])):
                swing_low = get_recent_swing_low_v2(df, i)
                ...
                swing_high = get_recent_swing_high_v2(df, i)
...
def analyze_results_v2(trades, df):
    if not trades:
        return pd.DataFrame()
