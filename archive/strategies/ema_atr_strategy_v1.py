...
def calculate_indicators(df):
    """Calculate EMA and ATR indicators and add them to the DataFrame."""
...
def find_swing_points(df, window=2):
    """Find swing highs and lows and mark them in the DataFrame."""
...
def get_recent_swing_low(df, current_idx, lookback=10):
    """Return the most recent swing low within the lookback period."""
...
def get_recent_swing_high(df, current_idx, lookback=10):
    """Return the most recent swing high within the lookback period."""
    start_idx = max(0, current_idx - lookback)
    recent_data = df.iloc[start_idx:current_idx+1]
    return recent_data['High'].max()

def backtest_strategy(df):
    """Run the EMA/ATR backtest and return trades and updated DataFrame."""
    trades = []
    position = None
...
def analyze_results(trades, df):
    """Return a DataFrame with trade results and statistics."""
    if not trades:
        return pd.DataFrame()
